...
app.config['MONGO_URI'] = "mongodb://21090079:21090079@localhost:27017/?authSource=auth"
...
client = MongoClient(
    'mongodb://21090079:21090079@localhost:27017/?authSource=auth')
db = client['21090079']
collection = db['users']
...
class User(UserMixin):
    ...
    def update_password(self, new_password):
        ...
        collection.update_one({'_id': ObjectId(self.id)}, {
...
@app.route('/')
def index():
    return jsonify('HELLLOW WORLDDD')
# def record_video():
#     cap = None
#     out = None
...
def update_password():
    try:
        ...
        user_data = collection.find_one({"_id": ObjectId(current_user.id)})
        if not user_data:
...
def change_email():
    try:
        ...
        existing_user = collection.find_one({"email": new_email})
        if existing_user:
...
def confirm_change_email():
    ...
    if not new_email:
        return {"message": "New email not provided"}, 400
    ...
    try:
        ...
        # Update the email address and verification status
        collection.update_one(
            {"_id": ObjectId(user_id)},
            {"$set": {"email": new_email, "is_email_verified": True}}
        )
        return jsonify({"message": "Email verification and update successful"}), 200
...
def change_password():
    ...
    hashed_password = bcrypt.generate_password_hash(
        new_password).decode('utf-8')
    collection.update_one({'_id': user_data['_id']}, {
        '$set': {'password': hashed_password}})
...
if __name__ == '__main__':
    app.run(debug=True, host='194.31.53.102', port=21079)
